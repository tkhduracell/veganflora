...
async function findFiles () {
  const files = await glob('./**/*.md', {
    cwd, ignore: ['_app', '_script', 'README.md']
  })
  return files.map(f => ({ file: path.join(cwd, f), dir: path.dirname(f), name: path.basename(f) }))
}

async function fileContents (files: { file: string, dir: string, name: string }[]) {
...
function parseRecipie (f: {content: string, file: string, dir: string, name: string}) {
