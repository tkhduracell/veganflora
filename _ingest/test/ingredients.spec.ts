...
describe('ingredients.ts', () => {
  it('parseIngredient(): parse four parts', () => {
    ...
    expect(out).toStrictEqual({ amount: '1 ½', measure: 'dl', name: 'kokande vatten' })
  })
  ...
  it('parseIngredient(): parse two parts with adjectives', () => {
    ...
    expect(out).toStrictEqual({ amount: '3', measure: 'stora', name: 'söta äpplen' })
  })

  it('parseIngredient(): parse two parts with adjectives', () => {
    const out = parseIngredient('30-40 g urkärnade dadlar (eller 1-2 tsk lönnsirap)')

    expect(out).toStrictEqual({ amount: '30-40', measure: 'g', name: 'urkärnade dadlar', note: 'eller 1-2 tsk lönnsirap' })
  })
