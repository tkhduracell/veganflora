<template>
  <div>
    <span v-for="(c, idx) in value" :key="c">
      ...
      <b-form-tag small :variant="variant" @remove="remove(idx)">
        {{ c }}
      </b-form-tag>
    </span>
    <span v-if="value && value.length > 0" class="separator">{{separator}}</span>

    <input v-if="custom" v-model="newTag" class="ml-1" @blur="complete()" @keypress.enter="complete()"/>
    <select v-else-if="suggestions.length > 0" size="sm" v-model="suggestion" @change="complete()">
      <option value="">Välj...</option>
      <option v-for="s in suggestions" :value="s" :key="s">
        {{ s }}
      </option>
    </select>
    <b-link @click="custom = !custom" class="ml-1 p-1">
      {{ custom ? '⨯' : '✎' }}
    </b-link>
  </div>
</template>
...
export default defineComponent({
  props: {
    ...
    suggestions: Array as PropType<string[]>,
