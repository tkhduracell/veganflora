...
describe("recipe tools", () => {
	...
	it("ranks search hits by vector distance", async () => {
		...
		const result = await client.callTool({
			name: "search_recipes",
			arguments: { query: "värmande soppa" },
		});
		expect(result.isError).toBeFalsy();

		const { results } = result.structuredContent as {
			results: Array<{ title: string; distance: number }>;
		};
