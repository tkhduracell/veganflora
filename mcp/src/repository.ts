...
export class RecipeRepository {
	...
	async listRecipes(opts: {
		...
		if (opts.category)
			query = query.where("category", "array-contains", opts.category);
	...
	async createRecipe(recipe: Recipe): Promise<{ id: string }> {
		...
		if (existing.exists) {
			throw new RecipeExistsError(
				`A recipe already exists at "${id}". Use update_recipe to change it.`,
			);
		}
	...
	async updateRecipe(id: string, patch: Partial<Recipe>): Promise<void> {
		...
		const existing = await ref.get();
		if (!existing.exists)
			throw new RecipeNotFoundError(`No recipe with id "${id}".`);
