...
import type { Ingredient, Recipe, Tag } from "@/components/types"

import { autoTag } from "../../modules/tags"
import { v4 as uuidv4 } from "uuid"

function asArray<V>(result: QuerySnapshot): { key: string; value: V }[] {
	return result.docs.map((itm) => ({ key: itm.id, value: itm.data() as V }))
}
...
export function useRecipe(key: string) {
	...
	function safekey(title: string, cats: string[]): string {
		const safe = (str: string) =>
			str.replace(/\//gi, "\\").replace(/ö/gi, "oe").replace(/ä/gi, "ae").replace(/å/gi, "aa").replace(/[\W]+/gi, "_")
	...
	function addIngredientRow() {
		...
		recipe.value = {
			...rest,
			ingredients: [...ingredients, { name: "", id: uuidv4() } satisfies Ingredient],
		}
	}

	function addIngredientRows(items: Omit<Ingredient, "id">[]) {
		const keyed = items.map((i, _idx) => ({ ...i, id: uuidv4() }))
		const { ingredients, ...rest } = recipe.value
		recipe.value = {
			...rest,
...
export function useRecipes(): Recipes {
	...
	onMounted(async () => {
		...
		const tags = await getDoc(doc(store, "veganflora", "root")).then(
			(itm) => (itm.data()?.prefill?.tags as Tag[]) ?? [],
		)
